...
class X402nService {
  ...
  private rankMockOffers(candidates: ProviderCandidate[], policy: NegotiationPolicy): ProviderOffer[] {
    return candidates
      .map((candidate, idx) => {
        const priceUsdc = this.resolveCandidatePrice(candidate, idx);
        const deliveryHours = this.resolveCandidateDeliveryHours(candidate, idx);
        const reputationScore = candidate.reputationScore ?? 650;
        const confidence = Number(Math.min(0.97, 0.74 + idx * 0.05).toFixed(2));
        const budgetScore = Math.max(0, 1 - priceUsdc / Math.max(policy.maxBudgetUsdc, 0.01));
        const latencyScore = Math.max(0, 1 - deliveryHours / Math.max(policy.maxDeliveryHours, 1));
        const normalizedReputationScore = Math.max(0, reputationScore / 1000);
        const score = Number(
          (budgetScore * 0.45 + latencyScore * 0.25 + normalizedReputationScore * 0.3).toFixed(3)
        );
        ...
        return {
          ...
          confidence,
          score,
          terms: `${candidate.serviceName} | ${deliveryHours}h SLA | ${candidate.source} catalog`,
          round: 0,
  ...
  private resolveCandidatePrice(candidate: ProviderCandidate, idx: number): number {
    const base = Number(candidate.basePriceUsdc ?? 0.1);
    if (!Number.isFinite(base)) {
      return Number((0.1 + idx * 0.01).toFixed(3));
    }
    return Number((base + idx * 0.0025).toFixed(3));
  }

  private resolveCandidateDeliveryHours(candidate: ProviderCandidate, idx: number): number {
    const text = `${candidate.serviceName} ${candidate.description}`.toLowerCase();
    if (text.includes('benchmark')) return 18 + idx * 2;
    if (text.includes('automation')) return 16 + idx * 2;
    if (text.includes('research')) return 24 + idx * 2;
    if (text.includes('image')) return 10 + idx * 2;
    return 20 + idx * 2;
  }
